    ...
    if tag == CAUSAL:
        ...
        for i in range(len(x[0])):
            ...
            if(x[0][(x[1]-1)] == (instance._vetorclock[(x[1]-1)]+1)) and (x[0][i] <= instance._vetorclock[i]):
                ...
                pass #raise GroupException
            else:
                pass #raise GroupException

#CODIGO ADICIONADO 17/06/2019 L.S. #18/06/2019
...
def join (instance, tag, tag_data, body):
    if tag == JOIN:
        ...
        instance._vetorclock.append(0) #CODIGO ADICIONADO 17/06/2019 L.S. #18/06/2019
        for member in instance._members:
...
def view (instance, tag, tag_data, body):
    if tag == VIEW:
        ...
        if len(instance._members) > len(members):
            instance._vetorclock.pop(instance._members.index([x for x in instance._members if x not in members].pop())) #CODIGO ADICIONADO 17/06/2019 L.S. #18/06/2019
